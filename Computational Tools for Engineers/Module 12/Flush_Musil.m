...
%% do a large number of hands and determine if the hand is 3 of a kind or a flush
TreKnd=0;                                              % initialize three of a kind counter
Flush = 0;                                              % Initialize flush counter
...
for i=1:Len                                            % for each hand   
    ...
    % Calculate the number of cards for each suit
    kSut = kHnd(:,4);
    nSut = zeros(4, 1);
    for iSut = 1:4; % For each suit
        fSut = (kSut==iSut);
        nSut(iSut) = sum(fSut);
    end
    
    % Decide if this is a flush
    nSutSort = sortrows(nSut, -1);     % Sort the suit from highest to lowest
    if nSutSort(1) == 5                    % For flush there must be a 5 in the first element of the  sorted suit array
        Flush = Flush + 1 ;
        fprintf('%2d %ld    ', kHnd(:, 3:4)'); fprintf('\n');
    end
...
NumPrbFlush = Flush/Len;
AnaPrbFlush = 5108/2598960;
...
WikPrbFlush = 1/508.8019;
fprintf('Three of a kind ---> Num Prb: %10.8f, True Prb: %10.8f \n', NumPrbTreKnd, AnaPrbTreknd);
fprintf('Flush ---> Num Prb: %10.8f, True Prb: %10.8f \n', NumPrbFlush, AnaPrbFlush);
tmp=0;